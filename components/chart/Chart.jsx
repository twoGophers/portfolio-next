...
export default function Chart({ width, height, data }) {
    ...
    const pie = useMemo(() => {
      const pieGenerator = d3.pie().value((d) => d.persent);
    ...
    const shapes = pie.map((grp, i) => {
      ...
      const labelPosX = inflexionPoint[0] +  ( width === 400 ?  10 : 50 ) * (isRightLabel ? 1 : -1);
      const textAnchor = isRightLabel ? "start" : "end";
      const label = grp.data.text + " (" + grp.data.persent + ")";
